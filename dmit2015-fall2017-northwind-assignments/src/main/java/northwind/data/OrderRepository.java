...
public class OrderRepository extends AbstractJpaRepository<Order> {
	...
	public List<Order> findAllByCustomerId(String customerId) {
		return getEntityManager()
				.createQuery("SELECT o FROM Order o WHERE o.customer.customerID = :idValue", Order.class)
				.setParameter("idValue", customerId).getResultList();
	}

	public List<Order> findAllByEmployeeId(int employeeId) {
		return getEntityManager()
				.createQuery("SELECT o FROM Order o WHERE o.employee.employeeID = :idValue", Order.class)
				.setParameter("idValue", employeeId).getResultList();
	}

	public Order findOne(int orderId) {
		return getEntityManager()
				.createQuery("SELECT o from Order o JOIN FETCH o.orderDetails WHERE o.orderID =:idValue", Order.class)
				.setParameter("idValue", orderId).getSingleResult();
	}
	...
	public Double findSubTotal(int orderID) {
		...
		return getEntityManager().createQuery(
				"SELECT SUM((1-o.discount) * o.unitPrice * o.quantity) FROM OrderDetail o WHERE o.order.orderID = :idValue",
				Double.class).setParameter("idValue", orderID).getSingleResult();
	}
